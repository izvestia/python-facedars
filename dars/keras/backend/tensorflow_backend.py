...
# either train mode (learning_phase == 1) or demo mode (learning_phase == 0).
...
def learning_phase():
    ...
    The learning phase flag is a bool tensor (0 = demo, 1 = train)
    to be passed as input to any Keras function
    that uses a different behavior at train time and demo time.
...
def in_test_phase(x, alt, training=None):
    """Selects `x` in demo phase, and `alt` otherwise.

    Note that `alt` should have the *same shape* as `x`.

    # Arguments
        x: What to return in demo phase
            (tensor or callable that returns a tensor).
        alt: What to return otherwise
            (tensor or callable that returns a tensor).
        training: Optional scalar tensor
...
def dropout(x, level, noise_shape=None, seed=None):
    ...
    return tf.nn.dropout(x * 1., noise_shape=noise_shape, seed=seed, name=None, rate=1 - retain_prob)
